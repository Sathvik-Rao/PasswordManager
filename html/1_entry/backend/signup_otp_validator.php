<?php session_start(); ?>
<?php
	if(isset($_POST['signup_email']) && filter_var($_POST['signup_email'], FILTER_VALIDATE_EMAIL) && isset($_POST['signup_password']) && isset($_POST['signup_code']) && isset($_POST['resend_code']) && isset($_POST['captcha_code']))
	{
		...
		if(mb_strlen($email, "UTF-8") < 257 && mb_strlen($password, "UTF-8") < 257 && mb_strlen($code, "UTF-8") < 7 && mb_strlen($resend_code, "UTF-8") < 2)
		{
			...
			$sql = "SELECT status, otp, otp_expiry, otp_failure_attempts FROM user_details WHERE email='$email' and password='$password'";
				...
				{	
					...
					if($resend_code == 0)	// user clicked on submit button
					{
						if(time_placeholder() < $row["otp_expiry"]) // otp didn't expired
						{
							...
							{
								if(mysqli_query($conn, "UPDATE user_details SET status=1, otp=0, otp_expiry=0, otp_failure_attempts=0, login_failure_attempts=0 WHERE email='$email'"))
								{
									echo '{"status":1}';
								}
							}
