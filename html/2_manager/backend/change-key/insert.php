		...
		try
		{
			...
			if($result && mysqli_num_rows($result) == 1) // valid user
			{
				...
				if(isset($_COOKIE['id']) && $_COOKIE['id'] != "" && is_numeric($_COOKIE['id']))	
				{
					...
					{
						$sql = "SELECT COUNT(*) AS count FROM `". $decoded->email ."_ck`";
						$result = mysqli_query($conn, $sql);
							...
							if(($used_rows-1) <= $max_rows)	// valid to insert new row
							{
								...
								else
								{
									...
									if($result && mysqli_num_rows($result) == 1)	// validate true id
									{
										...
										$sql = "SELECT SUM(size_in_bytes) AS size_in_bytes FROM `". $decoded->email ."_ck`";
										$result = mysqli_query($conn, $sql);
										if($result && mysqli_num_rows($result) == 1)	// get file(s) size
										{
											...
											{
												require '../../../files.php';
												array_map('unlink', array_diff(glob($path . $decoded->email . "_ck/*_temp"), array($path . $decoded->email . "_ck/" . $id . "_temp"))); // remove temp files except present id
												if(isset($_COOKIE['append']) && $_COOKIE['append'] != "")	// multiple-in
												{ 
													if($_COOKIE['append'] == "1")	// start and middle
													{
														$file = fopen($path . $decoded->email . "_ck/" . $id . "_temp", "a");
														if($file)
														{
															if(fwrite($file, $data))
															{
																...
																else
																{
																	...
																	unlink($path . $decoded->email . "_ck/" . $id . "_temp");
																}
															}
															else
															{
													...
													else if($_COOKIE['append'] == "2")	// end
													{
														...
														if($file)
														{
															if(fwrite($file, $data))
															{
																if(fclose($file))
																{
																	@unlink($path . $decoded->email . "_ck/" . $id);
																	if(rename($path . $decoded->email . "_ck/" . $id . "_temp", $path . $decoded->email . "_ck/" . $id))
																	{
																		if(mysqli_query($conn, "INSERT INTO `". $decoded->email ."_ck` (id, header, size_in_bytes) VALUES (". $id .", '". $header ."', ". ($temp_data_size + $received_bytes) .")"))
																		{
																			mysqli_query($conn, "DELETE FROM `". $decoded->email ."_temp`");
																			mysqli_close($conn);
																			ob_clean();
												...
												{
													$file = fopen($path . $decoded->email . "_ck/" . $id, "w");
													if($file)
													{
														if(fwrite($file, $data))
														{
															if(mysqli_query($conn, "INSERT INTO `". $decoded->email ."_ck` (id, header, size_in_bytes) VALUES (". $id .", '". $header ."', ". $received_bytes .")"))
															{
																fclose($file);
																mysqli_query($conn, "DELETE FROM `". $decoded->email ."_temp`");
																mysqli_close($conn);
														...
														{
															mysqli_query($conn, "DELETE FROM `". $decoded->email ."_ck` WHERE id=". $id);
															mysqli_query($conn, "DELETE FROM `". $decoded->email ."_temp`");
															fclose($file);
															unlink($path . $decoded->email . "_ck/" . $id);
														}
